CLASS zrap_abap_pvt_bp_employee DEFINITION PUBLIC ABSTRACT FINAL FOR BEHAVIOR OF zrap_abap_pvt_c_employee.
ENDCLASS.

CLASS zrap_abap_pvt_bp_employee IMPLEMENTATION.
  ...
  METHOD read.
    ...
    DATA lt_data TYPE TABLE OF zrap_abap_pvt_i_employee.

    IF keys IS NOT INITIAL.
      SELECT * FROM zrap_abap_pvt_i_employee
  ...
  METHOD checkdataquality.
    READ ENTITIES OF zrap_abap_pvt_c_employee IN LOCAL MODE
      ENTITY Employee
    ...
    LOOP AT lt_employee INTO DATA(ls_employee).
      ...
      DATA(lt_missing) = zrap_abap_pvt_cl_dq_evaluator=>evaluate(
  ...
  METHOD requestcorrection.
    ...
    LOOP AT lt_employee INTO DATA(ls_employee).
      DATA(lv_logged) = zrap_abap_pvt_cl_hr_update=>record_correction_request(
        VALUE #( personnel_number = ls_employee-PersonnelNumber
