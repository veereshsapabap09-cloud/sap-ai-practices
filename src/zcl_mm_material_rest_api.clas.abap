...
CLASS zcl_mm_material_rest_api IMPLEMENTATION.
  METHOD if_http_extension~handle_request.
    ...
    TRY.
        ls_request = deserialize_request( lv_json ).
    ...
    ls_response = process_request( ls_request ).

    set_json_response(
      io_server = server
      iv_status = 200
      iv_reason = 'OK'
      iv_json   = serialize_response( ls_response ) ).
  ENDMETHOD.
  ...
  METHOD call_material_bapi.
    ...
    "Maintain the views needed by the supplied upload template.
    ...
    IF rt_messages IS INITIAL.
      APPEND VALUE #(
        material_code = is_material-material_code
        type          = 'S'
        message       = COND #( WHEN material_exists( is_material-material_code ) = abap_true
                                THEN 'Material processed successfully'
                                ELSE 'Material creation/update accepted by BAPI' ) ) TO rt_messages.
    ENDIF.
  ENDMETHOD.
