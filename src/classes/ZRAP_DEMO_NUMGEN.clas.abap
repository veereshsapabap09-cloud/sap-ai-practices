...
CLASS zrap_demo_numgen IMPLEMENTATION.
  METHOD increment_counter.
    ...
    UPDATE zrap_demo_nr SET current_value = current_value + 1 WHERE object = @lv_object.
    lv_rows = sy-subrc.
    IF sy-subrc = 0.
      " Read the updated value
      SELECT SINGLE current_value FROM zrap_demo_nr INTO @rv_value WHERE object = @lv_object.
    ELSE.
      " No row existed yet; create initial row with value 1
      rv_value = 1.
      INSERT zrap_demo_nr VALUES ( lv_object rv_value ).
      IF sy-subrc <> 0.
