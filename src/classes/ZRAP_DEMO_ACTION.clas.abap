...
CLASS zrap_demo_action IMPLEMENTATION.
  METHOD activate_department.
    ...
    SELECT SINGLE * FROM zrap_demo_dept INTO @ls_dept WHERE departmentid = @iv_deptid.
    IF sy-subrc <> 0.
      rv_success = abap_false.
      RETURN.
    ENDIF.
    ls_dept-status = zrap_demo_const=>c_status_active.
    ls_dept-changedby = sy-uname.
    ls_dept-changedon = sy-datum.
    UPDATE zrap_demo_dept FROM ls_dept.
  ...
  METHOD close_department.
    ...
    IF sy-subrc <> 0.
      ...
      RETURN.
    ENDIF.
    ls_dept-status = zrap_demo_const=>c_status_closed.
    ls_dept-changedby = sy-uname.
    ls_dept-changedon = sy-datum.
    UPDATE zrap_demo_dept FROM ls_dept.
    rv_success = sy-subrc = 0.
  ...
  METHOD promote_employee.
    ...
    SELECT SINGLE * FROM zrap_demo_emp INTO @ls_emp WHERE departmentid = @iv_deptid AND employeeid = @iv_empid.
    IF sy-subrc <> 0.
      RETURN.
    ENDIF.
    ...
    UPDATE zrap_demo_emp FROM ls_emp.
    rv_success = sy-subrc = 0.
  ENDMETHOD.
