...
CLASS zt_demo_val_test IMPLEMENTATION.
  ...
  METHOD test_validate_department_name.
    DATA(lv_err) = zrap_demo_val=>validate_department_name( iv_name = '' ).
    cl_abap_unit_assert=>assert_not_initial( lv_err ).

    lv_err = zrap_demo_val=>validate_department_name( iv_name = 'Engineering' ).
    cl_abap_unit_assert=>assert_initial( lv_err ).
  ENDMETHOD.

  METHOD test_validate_employee_name.
    DATA(lv_err) = zrap_demo_val=>validate_employee_name( iv_name = '' ).
    cl_abap_unit_assert=>assert_not_initial( lv_err ).

    lv_err = zrap_demo_val=>validate_employee_name( iv_name = 'Jane Doe' ).
    cl_abap_unit_assert=>assert_initial( lv_err ).
  ENDMETHOD.

  METHOD test_validate_joining_date.
    DATA(lv_err) = zrap_demo_val=>validate_joining_date( iv_date = sy-datum + 1 ).
    cl_abap_unit_assert=>assert_not_initial( lv_err ).

    lv_err = zrap_demo_val=>validate_joining_date( iv_date = sy-datum ).
    cl_abap_unit_assert=>assert_initial( lv_err ).
  ENDMETHOD.
ENDCLASS.
