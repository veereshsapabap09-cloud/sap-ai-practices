CLASS zt_demo_numgen_db_test DEFINITION FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS.
  PRIVATE SECTION.
...
CLASS zt_demo_numgen_db_test IMPLEMENTATION.
  METHOD test_generate_ids.
    DATA(lv_did) = zrap_demo_numgen=>generate_department_id( ).
    DATA(lv_eid) = zrap_demo_numgen=>generate_employee_id( ).
